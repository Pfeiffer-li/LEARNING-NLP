...
        其次整体的监督式机器学习的过程就是：首先**拿到特征集合X**，然后把特征集合X**投入到预测函数**（prediction function）中运行，在运行过程中我们需要**投入输入参数 $\theta$**，最终也就**获得了模型的预测标签$`\widehat{y}`$**，然后用损失函数（cost function）对比预测标签$`\widehat{y}`$与集合标签Y之间的差异。这个预测函数的处理过程就是我们的模型处理过程。要想我们的模型的效果不错，需要**对我们的参数进行调整，然后重复上述步骤**，直到得出这个差异的最小值，最后就得到了最佳的模型。这里我们画个图加深理解：
...
        首先：初始化$`\theta`$，之后将它带入到sigmod函数中进行运算，之后将得出的结果通过cost损失函数来得出最小开销，最后**用梯度下降算法**来更新你的$`\theta`$；如果迭代多次，直到得出$`\theta`$的某一个值带入sigmod函数中，最后的计算结果带入损失函数得出的结果最小，那么这时的$`\theta`$的值才是**局部最优解**。

        其中为什么是局部最优解以及梯度下降算法会在该文件夹下另一个梯度下降算法的文章中介绍！！！这其中涉及到高等数学知识，如果对这方面不感兴趣的可以不用看这篇文章，不会影响后续的学习！！！

        **6、第六步：如何测评模型**

        首先，有一个完全正确的验证集，这个验证集的数据量是比实际的数据量小很多的；其次，需要将这个验证集投入到模型中进行预测，并且通过调参$`\theta`$，得出预测结果列表；之后，与你的实际验证集的结果列表进行同或比较（0与0得1，1与1得1，其他情况为0），得出最终得一个结果列表，计算这个结果中，为1的个数是多少，这个个数计为count，之后通过$`count/验证集总数 * 100\%`$，就得出了最终的准确率。

```textile
假如有5条验证集，实际的情感结果是：[0, 1, 1, 0, 1]
之后将这5条数据投入到模型中得到的预测结果列表是：[0, 1, 0, 0, 1]
那么将这两个列表进行同或计算，得出的最终的结果列表是：[1, 1, 0, 1, 1]
可以看出有4个为1的预测结果，计算得出模型准确率为：4/5 * 100% = 80%
```
...
        这个是在LR分类器中的损失函数，看上去很复杂，但是拆开来看就比较简单。

        首先了解这一部分$`-\frac{1}{m}\sum_{i=1}^{m}`$，它表示对所有你的推文数据结果之和取平均值，那么符号是什么含义呢？暂时先不管，往下看。

        之后，对中括号里面的内容做一个分析：首先看加号的左边，$`y^{(i)}`$表示这条数据在集合标签y中是积极还是消极（即0或1），想要加号左半边剩下，那么$`y^{(i)}`$只能是1的时候；那么再分析，$`h(x^{i}, \theta)`$表示你的预测结果，如果你的预测结果是1，则log值则会为0，所以加号左边的整体的值就是为0。如果你的$`y^{(i)}`$是1，则加号右边的整体的值自然为0，因为0乘以任何值都为0。也就是说，按刚所说的你的标签是1，且预测函数的结果是1的时候，损失函数的值只体现在加号左边，也就是这一条数据的损失为0。其他情况列在下表，可以自行分析一下：

| 情况                   | $`y^{i}`$ | $`logh(x^{i}, \theta)`$ | $`1-y^{i}`$ | $`log(1-h(x^{i}, \theta)`$ | 整体的值体现             |
| -------------------- | --------- | ----------------------- | ----------- | -------------------------- | ------------------ |
| label=0,preduction=0 | 0         | $`-\infty`$             | 1           | 0                          | 预测正确所以没损失          |
| label=0,preduction=1 | 0         | 1                       | 1           | $`-\infty`$                | 预测完全错误，所以这条数据损失无穷大 |
| label=1,preduction=0 | 1         | $`-\infty`$             | 0           | 0                          | 预测完全错误，所以这条数据损失无穷大 |
| label=1,preduction=1 | 1         | 0                       | 0           | $`-\infty`$                | 预测正确所以没损失          |

        当然，**我们的预测肯定不是一直都是0或者1，它的值是介于0与1之间的，所以$`log`$就会把预测值的结果固定在了负数**，所以在之前求和取平均值之前会添加一个符号，保证损失函数的损失值是永远是正值！

        综上，就是对损失函数的理解，也是整个LR分类器的完整的实现过程！
