...
        首先：初始化$`\theta`$，之后将它带入到sigmod函数中进行运算，之后将得出的结果通过cost开销函数来得出最小开销，最后**用梯度下降算法**来更新你的$`\theta`$；如果迭代多次，直到得出$`\theta`$的某一个值带入sigmod函数中，最后的计算结果带入开销函数得出的结果最小，那么这时的$`\theta`$的值才是**局部最优解**。
